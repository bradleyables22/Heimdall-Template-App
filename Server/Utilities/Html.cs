...
namespace Server.Utilities
{
    public static partial class Html
    {
        ...
        // ============================================================
        // Conditional helpers (for attributes and content)
        // ============================================================

        /// <summary>
        /// Conditionally includes a single part (attribute or content).
        /// Returns null when false so your Tag(...) can ignore it.
        /// </summary>
        public static object? When(bool condition, object? part)
            => condition ? part : null;

        /// <summary>
        /// Conditionally includes multiple parts. Returns null when false.
        /// Use this to avoid Html.Fragment(...) for attributes.
        /// </summary>
        public static object? When(bool condition, params object?[] parts)
        {
            if (!condition || parts is null || parts.Length == 0)
                return null;

            // Returning an array is fine as long as ElementNode.Create flattens nested enumerables.
            return parts;
        }

        /// <summary>
        /// Conditionally includes one of two parts.
        /// </summary>
        public static object? WhenElse(bool condition, object? whenTrue, object? whenFalse)
            => condition ? whenTrue : whenFalse;

        /// <summary>
        /// Conditionally includes one of two sets of parts.
        /// </summary>
        public static object? WhenElse(bool condition, object?[] whenTrue, object?[] whenFalse)
            => condition ? (object?)whenTrue : whenFalse;

        /// <summary>
        /// Includes the part only when it is not null.
        /// Handy for: Html.Maybe(user?.Name, v => Html.Span(v))
        /// </summary>
        public static object? Maybe<T>(T? value, Func<T, object?> factory) where T : class
            => value is null ? null : factory(value);

        /// <summary>
        /// Includes the part only when predicate is true.
        /// Handy for: Html.Maybe(count, c => c > 0, c => Html.Badge(c))
        /// </summary>
        public static object? Maybe<T>(T value, Func<T, bool> predicate, Func<T, object?> factory)
            => predicate(value) ? factory(value) : null;

        /// <summary>
        /// Simple switch helper (first match wins).
        /// </summary>
        public static object? Switch<T>(T value, params (T Match, object? Part)[] cases)
        {
            foreach (var (match, part) in cases)
            {
        ...
        /// <summary>
        /// Like Switch, but with a default.
        /// </summary>
        public static object? Switch<T>(T value, object? @default, params (T Match, object? Part)[] cases)
        {
            var hit = Switch(value, cases);
            return hit ?? @default;
        }

        /// <summary>
        /// Common pattern: include something when enumerable has any elements.
        /// Avoids multiple enumeration if you pass ICollection.
        /// </summary>
        public static object? WhenAny<T>(IEnumerable<T>? seq, Func<IEnumerable<T>, object?> factory)
        {
            if (seq is null) return null;
            if (seq is ICollection<T> c) return c.Count == 0 ? null : factory(seq);
            using var e = seq.GetEnumerator();
            return e.MoveNext() ? factory(seq) : null;
        }
